...
fn visit_dir(dir: &Path, tree_map: &mut BTreeMap<String, String>) -> Result<()> {
	...
	let permissions = metadata.permissions();
	    ...
	    let hex_hash = blob_hash.as_ref().iter().map(|b| format!("{:02x}", b)).collect::<String>();
	    let (hex_hash, _) = hex_hash.split_at(40);

	    // 100644 blob 83baae61804e65cc73a7201a7252750c76066a30	test.txt
            tree_map.insert(file_name.clone(), format!(
		"{} {} {}",
		if is_executable { 100755 } else { 100644 },
		hex_hash,
		file_name
	    ));
        }
